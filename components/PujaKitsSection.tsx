...
export default function PujaKitsSection() {
  ...
  const scroll = (direction: "left" | "right") => {
    ...
    const firstCard = el.querySelector<HTMLElement>("[data-puja-card]");
  ...
  return (
    <section
      id="puja-kits"
      className="relative z-30 bg-[#080a0f] border-t border-amber-500/15 py-8 sm:py-32 px-4 sm:px-10 lg:px-16 overflow-hidden"
    >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-2xl mx-auto mb-4 sm:mb-20">
          ...
          <p className="text-zinc-400 font-subheading italic text-sm sm:text-xl mt-1.5 sm:mt-4 leading-relaxed">
            Authentic Vedic puja samagri, thoughtfully curated for sacred rituals at home.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-zinc-950/80 border border-zinc-800/80 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-full">
              ...
              <span className="text-[10px] sm:text-xs font-mono uppercase tracking-wider sm:tracking-widest text-amber-300/90">
                {pujaKitProducts.filter((p) => !p.isPlaceholder).length} Consecrated Kits •{" "}
                {pujaKitProducts.filter((p) => p.isPlaceholder).length} Upcoming
              </span>
            </div>
          </div>
        ...
        <div className="relative -mx-4 sm:mx-0 px-4 sm:px-0">
          ...
          >
            {pujaKitProducts.map((product) => {
              const isWishlisted = !!wishlist[product.id];

              if (product.isPlaceholder) {
                // Structured Placeholder Card for Upcoming Kits
                return (
                  <div
                    key={product.id}
                    data-puja-card
                    className="snap-start flex-shrink-0 w-[66vw] min-w-[210px] max-w-[250px] sm:w-[calc(50%-12px)] sm:min-w-0 sm:max-w-none lg:w-[calc((100%-48px)/3)] group relative bg-zinc-950/40 rounded-xl sm:rounded-2xl border border-zinc-800/60 hover:border-amber-500/30 p-3 sm:p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_36px_rgba(0,0,0,0.7)]"
                  >
                    <div>
                      {/* Top Visual Placeholder Frame */}
                      <div className="relative aspect-[4/3] sm:aspect-square w-full rounded-lg sm:rounded-xl bg-gradient-to-b from-zinc-900/60 via-zinc-950 to-black p-3 sm:p-6 flex flex-col items-center justify-center overflow-hidden mb-2.5 sm:mb-6 border border-zinc-800/50 group-hover:border-amber-500/20 transition-colors">
                        <span className="absolute top-2 left-2 sm:top-3 sm:left-3 text-[8px] sm:text-[10px] font-mono uppercase tracking-wider bg-zinc-800/80 text-amber-300/70 border border-zinc-700/60 px-1.5 sm:px-2.5 py-0.5 rounded-full z-10">
                          {product.badge}
                        </span>

                        <span className="absolute top-2 right-2 sm:top-3 sm:right-3 text-[8px] sm:text-[10px] font-mono text-zinc-600">
                          {product.code}
                        </span>

                        {/* Sacred Symbol Watermark */}
                        <div className="w-12 h-12 sm:w-24 sm:h-24 rounded-full border border-amber-500/10 bg-amber-500/5 flex items-center justify-center text-center p-2 group-hover:scale-105 group-hover:border-amber-500/25 transition-all duration-500">
                          <span className="text-lg sm:text-3xl text-amber-400/40 font-serif">
                            🕉️
                          </span>
                        </div>
                        <span className="text-[8px] sm:text-[10px] font-mono text-zinc-500 uppercase tracking-widest mt-1.5 sm:mt-3">
                          Sacred Offering
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 mb-1 sm:mb-2">
                        <span className="text-[9px] sm:text-[11px] font-mono uppercase tracking-wider text-amber-400/70 line-clamp-1">
                          {product.tagline}
                        </span>
                      </div>

                      {/* Product Name */}
                      <h3 className="font-serif text-sm sm:text-xl font-bold text-zinc-200 group-hover:text-amber-300/90 transition-colors line-clamp-1 sm:line-clamp-none leading-tight">
                        {product.name}
                      </h3>
                    ...
                    {/* Bottom Area */}
                    <div className="mt-3 sm:mt-6 pt-2.5 sm:pt-5 border-t border-zinc-800/60">
                      <div className="flex items-center justify-between mb-2.5 sm:mb-4">
                        <span className="text-[9px] sm:text-[11px] font-mono text-zinc-500 uppercase">
                          Starting From
                        </span>
                      ...
                      <button
                        type="button"
                        disabled
                        className="w-full py-2 sm:py-2.5 px-3 rounded-lg sm:rounded-xl bg-zinc-900/60 border border-zinc-800 text-zinc-500 text-[10px] sm:text-xs font-medium tracking-wider uppercase cursor-not-allowed text-center"
                      >
                        Coming Soon
                      </button>
                    </div>
                ...
                >
                  ...
                  <div>
                    <div className="relative aspect-[4/3] sm:aspect-square w-full rounded-lg sm:rounded-xl bg-gradient-to-b from-zinc-900 via-zinc-950 to-black p-2 sm:p-4 flex items-center justify-center overflow-hidden mb-2.5 sm:mb-6 border border-zinc-800/80 group-hover:border-amber-500/40 transition-colors">
                      ...
                      {/* Product Image or Sacred Neutral Placeholder */}
                      <div className="relative w-full h-full flex items-center justify-center">
                        {product.image ? (
                          ...
                          <div className="flex flex-col items-center justify-center text-center p-2 sm:p-4">
                            <div className="w-10 h-10 sm:w-20 sm:h-20 rounded-full border border-amber-500/20 bg-amber-500/5 flex items-center justify-center text-center mb-1.5 sm:mb-3 group-hover:scale-110 group-hover:border-amber-500/40 transition-all duration-500">
                              <span className="text-base sm:text-3xl text-amber-400/60 font-serif">
                                🕉️
                              </span>
                            </div>
        ...
        <div className="mt-5 sm:mt-8 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-1.5 sm:gap-2 text-zinc-500 text-[10px] sm:text-xs font-mono">
            ...
            <span>Drag or swipe horizontally to reveal upcoming Puja Kits</span>
          </div>
