...
export default function FeaturedRubyCard() {
  ...
  const updateCoordinates = useCallback((clientX: number, clientY: number, maxTilt: number = 12) => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    updateCoordinates(e.clientX, e.clientY, 12);
  }, [updateCoordinates]);
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    ...
    updateCoordinates(touch.clientX, touch.clientY, 10);
    setIsHovered(true);
    playLuxuryHaptic();
  }, [updateCoordinates]);
